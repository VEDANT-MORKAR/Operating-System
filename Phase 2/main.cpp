...
class memory
{
	...
	public:
		void init()
		{
			...
			memset(occupied_pages,false,sizeof(bool)*29);
			page_table_ptr=rand()%29;
...
class cpu
{
	...
	public:
		...
		int address_map(int virtual_add)
		{
		...
		void startexe()
		{
			...
			while(!terminate)
			{
				...
				int pos=address_map(s_to_i(operand));
				TLC++;
		...
		void MOS()
		{
			...
			else if((TI==0 || TI==2) && SI==2)
			{
				...
				{
					int pos=address_map(s_to_i(operand)),flag=0;
					pos=(pos/10)*10;
					string ans="",temp="";
